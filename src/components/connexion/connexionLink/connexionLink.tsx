...
function ConnexionLink(props: { disconnectUser: () => void }) {
  const { disconnectUser } = props;
  const theme = useContext(themeContext) as Theme;
  const { currentColor } = theme;
  const settingsIcon = (
    <svg
      className="connexion__icon"
      viewBox="0 0 24 24"
      fill="none"
  ...
  const [connexionFormIsOpen, setConnexionForm] = useState(false);

  function toggleConnexionForm(connexionFormIsOpen: boolean) {
    setConnexionForm(!connexionFormIsOpen);
  }
  ...
  if (isConnected) {
    return (
      <button
        ...
        className="connexionLink__button"
      >
        {logout}
      </button>
    );
    ...
    return (
      <>
        <button
          className="connexionLink__button"
          onClick={() => toggleConnexionForm(connexionFormIsOpen)}
        >
          {settingsIcon}
        </button>
        {connexionFormIsOpen && (
          <ConnexionForm toggleConnexionForm={toggleConnexionForm} />
        )}
      </>
    );
  }
