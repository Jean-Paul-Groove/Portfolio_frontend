...
import EditCurriculum from "./editCurriculum/editCurriculum";
const cvURL = import.meta.env.VITE_CV_PATH_URL;

function Curriculum() {
  const token = useContext(AuthentifiedContext).token;
  const curriculumIcon = (
    <svg
      className="curriculum__icon"
  ...
  return (
    <>
      <a className="curriculum__link" href={cvURL} target="_blank">
        {curriculumIcon}
      </a>
      {token && <EditCurriculum />}
    </>
  );
}
